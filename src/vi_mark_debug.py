...
class InfiniteIBP(nn.Module):
    ...
    def elbo(self, X):
        ...
        if torch.isnan(a) or torch.isnan(b) or torch.isnan(c) or torch.isnan(d) or torch.isnan(e):
            print("term in elbo is nan")
    ...
    def _E_log_stick(tau, K):
        ...
        for k in range(K):
            ...
            if torch.isnan(torch_e_logstick).sum():
                print("The line above here nans when running spectrogram.py")
                import ipdb; ipdb.set_trace()
    ...
    def _2_feature_assign(self, nu, tau):
        ...
        """
        digamma_term = (digamma(tau[:,1]) - digamma(tau.sum(dim=1))).cumsum(0) 
        log_stick_term = self._E_log_stick(tau, self.K)[0]
        if torch.isnan(digamma_term).sum() or torch.isnan(log_stick_term).sum():
            print("digamma term or log stick term in _2_feature_assign() contains nan")
            import ipdb; ipdb.set_trace()
        
        return (nu * digamma_term +(1. - nu) * log_stick_term)
